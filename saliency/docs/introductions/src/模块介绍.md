...
```python 
#计算显著性区域wenjian
retval, saliencyMap = cv.saliency_StaticSaliencySpectralResidual.computeSaliency(image[, saliencyMap])
...
```python
#计算显著性区域
retval, saliencyMap = cv.saliency_StaticSaliencyFineGrained.computeSaliency(image[, saliencyMap])
